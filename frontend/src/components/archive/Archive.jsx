...
const Archive = ({ onView }) => {
  ...
  // Track last request for cancellation
  ...
  const fetchBarangays = async () => {
    try {
      ...
      const map = {};
      response.data.forEach((b) => (map[b.id] = b.barangay_name));
      setBarangayMap(map);
      setBarangayOptions([
        "All Barangays",
  ...
  // ─── Fetch Archived Citizens (with debounce + cancel) ─────────────
  const fetchArchivedCitizens = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        { params, withCredentials: true, signal: controller.signal }
      );
  ...
  const columns = [
    ...
    { label: "Archive Reason", key: "archive_reason" },
    { label: "Archived Date", key: "archive_date" },
    { label: "Deceased Date", key: "deceased_date" },
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      ...
      {/* Collapsible Filters */}
      {showFilters && (
        <div className="p-4 border-b border-gray-200 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
          <div>
            ...
            {loading ? (
              ...
              archivedCitizens.map((citizen) => (
                <tr key={citizen.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {citizen.barangay_id
                      ? `Brgy. ${barangayMap[citizen.barangay_id] || "Unknown"}`
